...
export default function TrainingApp({ testSlug }: { testSlug?: string }) {
  ...
  if (testSlug && !validTestSlug) return <TrainingFrame><div className={styles.onboarding}><h1>Prueba no encontrada</h1><Link className={styles.primaryButton} href="/dashboard/base12-training">Volver a Training</Link></div></TrainingFrame>;
...
function TrainingFrame({ children }: { children: React.ReactNode }) {
  return <div className={styles.page}><header className={styles.header}><div className={styles.headerInner}><Link href="/dashboard/base12-training" className={styles.brand}><Image src="/images/banco-opositores/logo-base12-training.png" alt="Base12 Training" width={70} height={70} priority /><span>Base12 Training</span></Link><nav className={styles.nav}><Link href="/dashboard/base12-training">Inicio</Link><Link href="/dashboard/base12-training#pruebas">Pruebas</Link><Link href="/dashboard/facturas">Mi cuenta</Link></nav></div></header>{children}</div>;
}
