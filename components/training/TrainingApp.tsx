...
function TrainingFrame({ children }: { children: React.ReactNode }) {
  return <div className={styles.page}><header className={styles.header}><div className={styles.headerInner}><Link href="/apps/base12-training" className={styles.brand}><Image src="/images/training/base12-training-192.png" alt="Base12 Training" width={70} height={70} priority /><span>Base12 Training</span></Link><nav className={styles.nav}><Link href="/apps/base12-training">Inicio</Link><Link href="/apps/base12-training#pruebas">Pruebas</Link><Link href="/dashboard/facturas">Mi cuenta</Link></nav></div></header>{children}</div>;
}
...
function TrainingDashboard({ sex, results }: { sex: TrainingSex; results: ResultRow[] }) {
  ...
  return <TrainingFrame><main className={styles.main}><section className={styles.hero}><div className={styles.heroIntro}><p className={styles.eyebrow}>Preparación física · Tropa y Marinería</p><h1>Tu entrenamiento, prueba a prueba</h1><p>Registra tus marcas, sigue el plan y deja que Carlos ajuste el siguiente objetivo y los ejercicios de cada sesión.</p></div><div className={styles.carlosCard}><Image className={styles.carlosAvatar} src="/images/training/carlos.webp" alt="Carlos, entrenador IA de Base12 Training" width={220} height={220} /><div><h2>Carlos · Entrenador IA</h2><p>He revisado tus cuatro pruebas. Hoy priorizaría <strong>{priority.name}</strong> para acercarnos a {formatTrainingValue(priority, priorityTarget)}.</p></div></div></section><section id="pruebas" className={styles.grid4}>{trainingTests.map((test) => { const rows = byTest[test.slug]; const values = rows.map((row) => Number(row.result_value)); const latest = values[0] ?? null; const best = getBestTrainingValue(test, values); const target = nextTrainingTarget(test, latest, sex); const official = getOfficialTarget(test, sex); const state = getTrainingProgressState(test, values, sex); return <Link key={test.slug} href={`/apps/base12-training/${test.slug}`} className={styles.testCard} style={testStyle(test)}><div className={styles.testIcon}>{testEmoji(test.slug)}</div><h3>{test.name}</h3><div className={styles.miniMetrics}><div><span>Última marca</span><strong>{formatTrainingValue(test, latest)}</strong></div><div><span>Mejor marca</span><strong>{formatTrainingValue(test, best)}</strong></div><div><span>Marca oficial</span><strong>{formatTrainingValue(test, official)}</strong></div><div><span>Objetivo Carlos</span><strong>{formatTrainingValue(test, target)}</strong></div></div><span className={styles.status}>{state}</span></Link>; })}</section><section className={`${styles.panel} ${styles.today}`}><div className={styles.todayTop}><div><p className={styles.eyebrow}>Hoy te toca</p><h2>{priority.name}</h2><p>Calentamiento obligatorio + trabajo específico. Carlos ajustará la selección de ejercicios con tus resultados recientes.</p></div><Link className={styles.primaryButton} href={`/apps/base12-training/${priority.slug}`}>Comenzar entrenamiento →</Link></div></section></main></TrainingFrame>;
}
...
function TrainingTestView({ testSlug, sex, results, recommendation, reload, notice, setNotice, saving, setSaving }: { testSlug: TrainingTestSlug; sex: TrainingSex; results: ResultRow[]; recommendation: Recommendation | null; reload: () => Promise<void>; notice: string; setNotice: (value: string) => void; saving: boolean; setSaving: (value: boolean) => void }) {
  ...
  const progressState = getTrainingProgressState(test, results.map((row) => Number(row.result_value)), sex);
