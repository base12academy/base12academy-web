...
export default function MatematicasIIGlossaryLauncher() {
  ...
  async function openGlossary() {
    ...
    const response = await fetch("/api/matematicas-ii/glossary", {
    ...
    if (response.ok) {
      ...
      setError("Inicia sesión para acceder al glosario completo de Matemáticas II.");
    } else if (payload?.error === "matriculation_required") {
      setError("El glosario completo requiere una modalidad con acceso a Matemáticas II.");
    } else {
  ...
  return <>
    <button
      ...
      aria-label="Abrir glosario de Matemáticas II"
      style={launcherStyle}
    >
      <span style={{ fontSize: 18 }}>Aa</span>
      <span><strong style={{ display: "block", fontSize: 13 }}>Glosario</strong><small style={{ display: "block", opacity: .78, fontSize: 10 }}>90 términos</small></span>
    </button>

    {open ? <div style={backdropStyle} role="presentation" onMouseDown={(event) => { if (event.currentTarget === event.target) setOpen(false); }}>
      <aside style={drawerStyle} role="dialog" aria-modal="true" aria-label="Glosario de Matemáticas II">
        <header style={headerStyle}>
          <div>
            <span style={eyebrowStyle}>RECURSO BASE12</span>
            <h2 style={{ margin: "5px 0 2px", fontSize: 24, color: "#0b2c5d" }}>Glosario de Matemáticas II</h2>
