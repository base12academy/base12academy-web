...
export type FilosofiaBlock =
  | { type: "heading"; level: number; text: string }
  | { type: "paragraph"; text: string }
  | { type: "list_item"; text: string }
  | { type: "table"; rows: string[][] };

export default function FilosofiaBlocks({ blocks }: { blocks: FilosofiaBlock[] }) {
  ...
  for (let index = 0; index < blocks.length; index += 1) {
    ...
    if (block.type === "list_item") {
      ...
      while (blocks[index + 1]?.type === "list_item") {
        index += 1;
        items.push((blocks[index] as Extract<FilosofiaBlock, { type: "list_item" }>).text);
      }
      rendered.push(
        <ul className={styles.documentList} key={`list-${index}`}>
          {items.map((item, itemIndex) => <li key={`${itemIndex}-${item.slice(0, 28)}`}>{item}</li>)}
        </ul>,
      );
      continue;
    }
