...
export default function WrittenAssessment({courseSlug,contentId,activityType,prompt,expectedAnswer="",rubric="",unit="",group="",rows=6}:Props){
  ...
  const [result,setResult]=useState<{score:number|null;feedback:string;expectedAnswer?:string;rubric?:string}|null>(null); const [error,setError]=useState("");
  async function submit(){
    if(!answer.trim()||sending)return; setSending(true); setError("");
    try{const data=await submitAssessment({courseSlug,contentId,activityType,prompt,answer,unit,group});setResult({score:data.score,feedback:data.feedback,expectedAnswer:data.expectedAnswer,rubric:data.rubric});}
  ...
  return <div style={{marginTop:14}}>
    ...
    {result?<div style={{marginTop:12,border:"1px solid #d6e5f2",background:"#f4f9ff",borderRadius:10,padding:13,color:"#173f70"}}><strong>{result.score==null?"Respuesta registrada":("Valoración orientativa: "+result.score+" / 100")}</strong><p style={{margin:"6px 0 0",lineHeight:1.5}}>{result.feedback}</p>{result.expectedAnswer?<details style={{marginTop:10}}><summary style={{cursor:"pointer",fontWeight:800,color:"#0b58ad"}}>Ver respuesta esperada y rúbrica</summary><p style={{whiteSpace:"pre-wrap",lineHeight:1.5}}>{result.expectedAnswer}</p>{result.rubric?<p style={{whiteSpace:"pre-wrap",fontSize:13,color:"#58718b"}}>{result.rubric}</p>:null}</details>:null}</div>:null}
  </div>;
}