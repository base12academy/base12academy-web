...
export async function GET(req: NextRequest) {
  ...
  const {data:bookings}=await ctx.supabase.from("tutoring_bookings").select("id,slot_id,status,tutoring_slots(starts_at,ends_at)").eq("user_id",ctx.user.id).eq("course_slug",course).eq("status","booked");
  return NextResponse.json({slots:slots||[],bookings:bookings||[]});
}
export async function POST(req: NextRequest) {
  const ctx=await auth(req); if(!ctx)return NextResponse.json({error:"unauthorized"},{status:401}); const body=await req.json().catch(()=>({})); const course=String(body.courseSlug||""); const slotId=String(body.slotId||""); const now=new Date().toISOString();
  ...
  const {data,error}=await ctx.supabase.from("tutoring_bookings").insert({slot_id:slotId,enrollment_id:e.id,user_id:ctx.user.id,course_slug:course}).select("id").single(); if(error)return NextResponse.json({error:"slot_unavailable"},{status:409}); return NextResponse.json({booking:data});
}
