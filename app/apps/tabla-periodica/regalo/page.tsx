...
function PeriodicTableGiftContent() {
  ...
  useEffect(() => {
    ...
    async function load() {
      const [{ data: userData }, response] = await Promise.all([
        ...
        code
          ? fetch(`/api/apps/tabla-periodica/gift/status?codigo=${encodeURIComponent(code)}`, { cache: "no-store" })
  ...
  const returnPath = useMemo(
    () => `/apps/tabla-periodica/regalo?codigo=${encodeURIComponent(code)}`,
    [code],
  );
  ...
  async function redeemGift() {
    ...
    const response = await fetch("/api/apps/tabla-periodica/gift/redeem", {
      method: "POST",
      headers: {
  ...
  return (
    <main className={styles.page}>
      <header className={styles.header}>
        ...
        <Link href="/apps/tabla-periodica">Conocer la aplicación</Link>
      </header>
      ...
      <section className={styles.card}>
        <div className={styles.visual}>
          <Image
            ...
            alt="Icono B12 Tabla Periódica Interactiva"
            width={260}
            height={260}
            priority
          />
          <Image
            className={styles.clara}
            src="/images/clara-tabla-periodica.png"
            alt="Clara, asistente de química"
            width={190}
            height={235}
          />
        </div>
        ...
        <div className={styles.content}>
          ...
          <h1>Tu Tabla Periódica Interactiva, de regalo</h1>
          <p className={styles.lead}>Acceso permanente a los 118 elementos y sus datos, búsqueda, fichas, tendencias, comparación y Clara como asistente de química.</p>
          ...
          {status.loading || checkingUser ? (
            ...
            <div className={styles.successBox}>
              <strong>La aplicación ya es tuya</strong>
              <p>Tu acceso permanente ha quedado asociado a {user?.email}.</p>
              <Link href="/apps/tabla-periodica" className={styles.primaryButton}>Abrir la Tabla Periódica</Link>
            </div>
          ) : !user ? (
...
export default function PeriodicTableGiftPage() {
  return (
    <Suspense fallback={<main style={{ padding: 32 }}>Comprobando tu regalo…</main>}>
      <PeriodicTableGiftContent />
    </Suspense>
  );
}
