...
function Course() {
  ...
  return (
    <div className={styles.shell}>
        ...
        {checking ? <section className={styles.locked}><div className={styles.lockIcon}>…</div><div><h2>Comprobando acceso</h2><p>Estamos verificando la matrícula de esta cuenta.</p></div></section> : !allowed || !content ? (
          ...
          <div className={layout.learningGrid}>
            ...
            <section className={styles.contentCard}>
              ...
              {tab === "rocio" && <><Rocio items={content.rocio} onChat={()=>setRocioChatOpen(true)} /><OpenQuestion courseSlug="administrativo-ja" themeId={theme.id} /></>}
              {tab === "fernando" && <><Assistant name="Fernando" role="Tutor IA" text={`Te ayuda a organizar el estudio del Tema ${theme.number}, comprobar tu avance y retomar el itinerario cuando lo necesites.`} onAction={()=>setFernandoChatOpen(true)} /><CourseProgressSummary courseSlug="administrativo-ja"/></>}
      ...
      <CourseAssistantChat entryPoint="rocio" courseSlug="administrativo-ja" contextTitle={theme.id+" · "+theme.title} open={rocioChatOpen} onClose={()=>setRocioChatOpen(false)}/>
      <CourseAssistantChat entryPoint="fernando" courseSlug="administrativo-ja" contextTitle={theme.id+" · "+theme.title} open={fernandoChatOpen} onClose={()=>setFernandoChatOpen(false)}/>
    </div>
  );
}
