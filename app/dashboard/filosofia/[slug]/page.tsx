...
function Locked({ reason, message, compact = false }: { reason?: string; message?: string; compact?: boolean }) {
  return <div className={compact ? styles.lockedCompact : styles.locked}><p className={styles.eyebrow}>ACCESO AL CURSO</p><h1>{message || (reason === "login_required" ? "Inicia sesión para continuar" : "Este contenido requiere una matrícula activa")}</h1><p>La unidad de Sócrates está abierta. El resto del material queda disponible para alumnos matriculados y administración.</p><div><Link href="/dashboard/filosofia/autor-socrates">Ver unidad abierta</Link><Link href="/login?redirect=/dashboard/filosofia">Iniciar sesión</Link><Link href="/#catalogo">Ver modalidades</Link></div></div>;
}
